...
def modify_command_options(opts):
    ...
    if opts.method is not None:
        ...
        if opts.method == 'ILT':
            ...
            opts.distill_segformer = False
            opts.distill_weight_args = 1
            
        # opts.balance_exemplar = False

        if opts.balance_exemplar:
            if opts.dataset == 'voc':
                opts.ssul_m_exemplar_total = 100
            elif opts.dataset == 'ade':
                opts.ssul_m_exemplar_total = 300
        ...
        if opts.method == 'SATS':
            """
            SATS, TRANSFER SELF-ATTENTION FROM SEGFORMER
            """
            opts.pseudo = "entropy"
            opts.threshold = 0.001
            opts.classif_adaptive_factor = True
            ...
            opts.distill_segformer = True
            opts.distill_object = 'sats-selfattn'
            
            opts.distill_weight_args = 20
            print(opts.distill_weight_args)

    opts.no_overlap = not opts.overlap
    opts.no_cross_val = not opts.cross_val
    opts.print_int = 1
    
    opts.small_sample_test = False
...
def get_argparser():
    ...
    parser.add_argument("--distill_object", type=str, default='v4', choices=
        ['sats-selfattn', 'sats-selfattn-GP', 'sats-selfattn-NP', 'sats-feature-CNN'])
    parser.add_argument("--small_sample_test", type=bool, default=False)
    parser.add_argument("--distill_weight_args", type=float, default=10)
    ...
    parser.add_argument("--model", type=str, choices=['deeplab', 'segformer_b2'])
    ...
    parser.add_argument(
        "--checkpoint", type=str, default="./checkpoints_model/step"
    )

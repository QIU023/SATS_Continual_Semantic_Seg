...
def save_ckpt(path, model, trainer, optimizer, scheduler, epoch, best_score, 
              sim_matrix=None, appear_times_total = None
        ):
    """ save current model
    """
    state = {
...
#     print(opts.step)
    ...
    num_old_cls = num_cls - num_cls_arr[-1]

    relevant_matrix = torch.zeros(num_cls, num_cls).to(device)
    appear_times_total = torch.zeros(num_cls, num_cls).to(device)
#     appear_times = torch.zeros(num_cls, num_cls)
...
#                 model.moinit_new_classifier(device)
...
#                 logger.info('')
    ...
    relevant_matrix = relevant_matrix.to(device)
    appear_times_total = appear_times_total.to(device)
...
#             print(cur_epoch)
            epoch_loss = trainer.train(
                ...
                relevant_matrix_combined=relevant_matrix,
                appear_times_total=appear_times_total
            )
                ...
                if rank == 0:  # save best model at the last iteration
                    ...
                    if score > best_score:
                        ...
                        save_ckpt(
                            ...
                            trainer, optimizer, scheduler, cur_epoch, score, relevant_matrix, appear_times_total
                        )

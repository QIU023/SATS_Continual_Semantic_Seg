    ...
    def __init__(self, dataset, indices, transform=None, target_transform=None):
        ...
        if self.in_memory:
            for i in tqdm(self.indices):
                tup = self.dataset.images[i]
                try:
                    self.dataset.images[i] = (
                        Image.open(tup[0]).convert("RGB"),
                        Image.open(tup[1])                    
                    )
                except:
                    continue
            print(f'put {len(self.indices)} images into memory!')
...
#         if (target == 21).sum() > 0:
#         print(torch.unique(tg0), torch.unique(target))

...
class StreamSegMetrics(_StreamMetrics):
    ...
    def get_results(self, valid_cls = None, print_task=None):
        ...
        mask = (gt_sum != 0)
#         mask = np.zeros(gt_sum.shape).astype(np.uint8)
        if valid_cls is not None:
            if 0 not in valid_cls:
                valid_cls += [0]
            for i in range(len(mask)):
                if i not in valid_cls:
                    mask[i] = False
#         print(valid_cls)
#         mask = [0, 16, 17, 18, 19, 20]
        diag = np.diag(hist)
...
#         cls_acc = dict(
#             zip(range(self.n_classes), acc_cls_c)
#         )
        
#         if print_task is not None:
#             ntask = len(print_task.keys())
#             old_iou = np.mean(acc_cls_c[print_task[0]])
#             new_iou = 0.
#             for i in range(1, ntask):
# #                 if i == 0:
#                 try:
#                     new_iou += np.mean(acc_cls_c[print_task[i]])
#                 except:
#                     break
#             new_iou /= (ntask-1)
#             print(f'all class miou:{mean_iu}, old class miou:{old_iou}, new class miou:{new_iou}')

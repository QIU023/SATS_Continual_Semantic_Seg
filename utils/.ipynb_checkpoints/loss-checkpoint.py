...
from plop_distill_func import difference_func
...
class ClassAttnSimilarityCrossEntropyLoss(nn.Module):
    
    def __init__(self, ignore_index=255, reduction='mean', num_cls=20, num_old_cls=15,
                     lambda_param=1, weight_mode='linear', max_epoch=30):
        super().__init__()
        self.ignore_index = ignore_index
        self.reduction = reduction
    ...
    def forward(self, device, inputs, targets, cls_attn_sim_matrix, epoch):

            
#         pass
#         with torch.autograd.set_detect_anomaly(True):
        cls_attn_sim_matrix[torch.isnan(cls_attn_sim_matrix)] = 0
        cls_attn_sim_matrix[torch.isinf(cls_attn_sim_matrix)] = 0
#         print(torch.unique(targets))
    
#         print(cls_attn_sim_matrix)
    
#         for c1 in range(self.num_cls):
# #             if cls_attn_sim_matrix[c1].sum() > 0:
# #                 cls_attn_sim_matrix[c1] /= cls_attn_sim_matrix[c1].sum()
#             cls_attn_sim_matrix[c1, c1] = 1
    
#         print(cls_attn_sim_matrix)
        
        c, c = cls_attn_sim_matrix.shape
        assert self.num_cls == c, "class similarity matrix dimension error!"
        assert self.num_cls == inputs.shape[1], "number class error!"
#         torch.autograd.set_detect_anomaly(True)

        cls_attn_sim_matrix = cls_attn_sim_matrix.detach()
        
        n_cl = torch.tensor(inputs.shape[1]).to(device)
        labels_new = torch.where(targets != self.ignore_index, targets, n_cl)
        # replace ignore with numclasses + 1 (to enable one hot and then remove it)
        onehot_targets = F.one_hot(labels_new, inputs.shape[1] + 1).float().permute(0, 3, 1, 2)
#         print(onehot_targets.shape, inputs.shape, targets.shape)
        onehot_targets = onehot_targets[:, :inputs.shape[1], :, :]  # remove 255 from 1hot
        
        use_offical_bce = False
        if use_offical_bce:
            standard_bce_loss = nn.BCEWithLogitsLoss(reduction='mean')
            return standard_bce_loss(inputs, onehot_targets)
...
#         print(onehot_targets.shape)
        ...
        for c in range(self.num_old_cls):
            if c == 0:
                weights_c = 1
            else:
            # PROBLEMATIC???????????
                weights_c = self.obtain_weight(device, c, targets, cls_attn_sim_matrix)
#             weights_c = 1
            loss_t = - onehot_targets[:, c]*(torch.log(inputs[:, c]+eps)) 
            loss_f = - weights_c * (1-onehot_targets[:, c]) *(torch.log(1-inputs[:, c]+eps))
            loss_c = (loss_t + loss_f).mean(dim=[1,2])
            loss += loss_c

        epoch_lambda = epoch / self.max_epoch 
        # with the rising of epoch, we trust the new class similarity more
        # therefore, epoch_lambda rising -> weights_c rising 

        for c in range(self.num_old_cls, self.num_cls):
            if c == 0:
                weights_c = 1
            else:
            # PROBLEMATIC???????????
                weights_c = self.obtain_weight(device, c, targets, cls_attn_sim_matrix)
#             weights_c = 1
#             print(onehot_targets[:, c, ], torch.log(inputs[:, c]+eps))
            loss_t = - onehot_targets[:, c]*(torch.log(inputs[:, c]+eps)) 
            loss_f = - ((1 - epoch_lambda) + epoch_lambda * weights_c) * (1-onehot_targets[:, c]) *(torch.log(1-inputs[:, c]+eps))
            
            loss_t[torch.isnan(loss_t)] = 0
            loss_f[torch.isnan(loss_f)] = 0
...
#         print(loss.grad.mean())
    ...
    def obtain_weight(self, device, c, targets, similarity_matrix):
        weights = torch.ones(targets.shape).to(device)
        tar_cls = torch.unique(targets)
        for ci in tar_cls:
            if ci == 255:
                continue
            area = targets == ci
            if similarity_matrix[c, ci] != 0:
                weights[area] = self.transform_weight(similarity_matrix[c, ci])          
#             print(similarity_matrix[c, ci])
#         weights = self.transform_weight(weights)
#         print(weights.shape)
        return weights
...
#         weight[0] = 1
        if self.weight_mode == 'linear':
            return 10*(0.5 + weight / 2)
        elif self.weight_mode == 'exp':
            return torch.exp(weight / self.lambda_param)
        
class ClassAttnSimilarityDistillationLoss(nn.Module):
    
    def __init__(self, ignore_index=255, reduction='mean', num_cls=20, normalize=True):
        super().__init__()
        self.ignore_index = ignore_index
        self.reduction = reduction
        self.feature_scale = [128, 64, 32, 16]
        self.reduction_rate = [8,4,2,1]
        self.difference_function = "frobenius"
        self.num_cls = num_cls
        self.normalize = normalize
    
    def forward(self, cls_attn_mean_prototype_arr, old_cls_attn_mean_prototype_arr, appear_cls):
#         pass
        loss = 0.
        for cls_new, cls_old in zip(cls_attn_mean_prototype_arr, old_cls_attn_mean_prototype_arr):
            ## cls_new C Hs*L/R^2
            sc_loss = 0.
            for cl in appear_cls:
                cl = int(cl.item())
                a, b = cls_new[cl], cls_old[cl]
                if self.normalize:
                    a = F.normalize(a, dim=0, p=2)
                    b = F.normalize(b, dim=0, p=2)
                sc_loss += difference_func(a, b, self.difference_function)
            sc_loss /= appear_cls.shape[0]
            loss += sc_loss
        loss /= len(cls_attn_mean_prototype_arr)
...
class SmallObjectAttnCrossEntropyLoss(nn.Module):
    
    def __init__(self, ignore_index=255, reduction='None', num_cls=20):
        super().__init__()
        self.ignore_index = ignore_index
        self.reduction = reduction
        self.num_cls = num_cls

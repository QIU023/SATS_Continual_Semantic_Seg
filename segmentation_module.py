...
import models
from modules import DeeplabV3, SegFormer_Body, SegFormer_Head, segformer_type_args, DSS, build_model
...
#         opts.norm_act = 'normal'
        ...
        if not opts.no_pretrained:
            ...
            body.load_state_dict(pre_dict, strict=False)
...
class IncrementalSegmentationModule(nn.Module):
    ...
    def __init__(
        ...
        if self.opts.method == 'SSUL' or self.opts.method == 'Innovation-v2':
            self.salient_detector = build_model()
            self.salient_detector.load_state_dict(torch.load('./pretrained/final_700.pth'))
            classes.insert(0, 1)
            classes.insert(0, 1)
            classes[2] -= 1
#             self.cls_unknown
        print('step num classes:',classes)
    ...
    def init_ssul_with_unknown(self, device):
        ### BG 0 UNKNOWN 1 OLD 2~N-1 NEW N
        unknown_weight = self.cls[1].weight.data
        num_new = self.cls[-1].weight.data.shape[0]
        cp_un_weight = torch.cat([unknown_weight]*num_new, dim=0)
        self.cls[-1].weight.data.copy_(cp_un_weight)
    ...
    def _network(self, x, ret_intermediate=False, only_bg=False):
        ...
        if self.opts.model == 'deeplab':
            ...
            attentions, ret_attn = self.body(x)
            x_pl, featuremap = self.head(attentions)
            x_b = attentions[-1]
        out = []
        ...
        if ret_intermediate:
            return x_o, x_b, x_pl, attentions, ret_attn, featuremap
        return x_o
    ...
    def forward(self, x, scales=None, do_flip=False, ret_intermediate=False, only_bg=False):
        ...
        if self.opts.method == 'SSUL' or self.opts.method == 'Innovation-v2':
            with torch.no_grad():
                salient_det_res = self.salient_detector(x)[-1]
            sem_logits = (sem_logits, salient_det_res)
        
        if ret_intermediate:
            if self.opts.model == 'segformer_b2':
                return sem_logits, {
                    "distill4segformer": out[4],
                    "featuremaps":out[3],
                    "decoder_out":out[2],

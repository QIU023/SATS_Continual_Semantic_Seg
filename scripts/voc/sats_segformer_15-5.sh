...
<<<<<<< HEAD:scripts/voc/sats_segformer_15-5.sh
CUDA_VISIBLE_DEVICES=${GPU} python3 -m torch.distributed.launch --master_port ${PORT} --nproc_per_node=${NB_GPU} run.py --date ${START_DATE} --data_root ${DATA_ROOT} --overlap --batch_size ${BATCH_SIZE} --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 1 --lr 0.001 --epochs ${EPOCHS} --method ${METHOD} --opt_level O1 ${OPTIONS} --model 'segformer_b2' --ckpt checkpoints_model/step/15-5-voc_SATS_1_segformer_b2.pth
=======
...
>>>>>>> 404191147e369eaf173cd2763051c90fcebe4998:scripts/voc/inno4_segformer_15-5.sh

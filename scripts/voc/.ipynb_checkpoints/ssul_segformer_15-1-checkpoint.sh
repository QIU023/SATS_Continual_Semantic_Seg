...
# CUDA_VISIBLE_DEVICES=${GPU} python3 -m torch.distributed.launch --master_port ${PORT} --nproc_per_node=${NB_GPU} run.py --date ${START_DATE} --data_root ${DATA_ROOT} --overlap --batch_size ${BATCH_SIZE} --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 0 --lr 0.01 --epochs ${INITIAL_EPOCHS} --method ${METHOD} --opt_level O1 ${OPTIONS} --model 'segformer_b2' --ckpt checkpoints/step/15-5s-voc_SSUL_0_segformer_b2.pth
...
# rm checkpoints/step/15-5s-voc_SSUL_1_segformer_b2.pth
# rm checkpoints/step/15-5s-voc_SSUL_2_segformer_b2.pth
# rm checkpoints/step/15-5s-voc_SSUL_3_segformer_b2.pth
# rm checkpoints/step/15-5s-voc_SSUL_4_segformer_b2.pth

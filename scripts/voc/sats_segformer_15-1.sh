...
# --ckpt checkpoints_model/step/5-3-voc_SATS_5_segformer_b2.pth
...
CUDA_VISIBLE_DEVICES=${GPU} python3 -m torch.distributed.launch --master_port ${PORT} --nproc_per_node=${NB_GPU} run.py --date ${START_DATE} --data_root ${DATA_ROOT} --overlap --batch_size ${BATCH_SIZE} --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 5 --lr 0.001 --epochs ${EPOCHS} --method ${METHOD} --opt_level O1 ${OPTIONS} --model 'segformer_b2' --ckpt checkpoints_model/step/15-5s-voc_Innovation-v4_5_segformer_b2.pth

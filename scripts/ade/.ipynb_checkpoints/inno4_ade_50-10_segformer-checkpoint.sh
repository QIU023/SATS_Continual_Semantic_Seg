...
OPTIONS="--loss_kd 20 --distill_weight_args 200 --distill_ver v4"
...
# CUDA_VISIBLE_DEVICES=${GPU} python3 -m torch.distributed.launch --master_port ${PORT} --nproc_per_node=${NB_GPU} run.py --date ${START_DATE} --data_root ${DATA_ROOT} --overlap --batch_size 12 --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 3 --lr 0.001 --epochs 60 --method ${METHOD} --opt_level O1 ${OPTIONS} --model 'segformer_b2' --distill_weight_args 30 --ckpt checkpoints_model/step/50-10-ade_Innovation-v4_3_segformer_b2.pth
CUDA_VISIBLE_DEVICES=${GPU} python3 -m torch.distributed.launch --master_port ${PORT} --nproc_per_node=${NB_GPU} run.py --date ${START_DATE} --data_root ${DATA_ROOT} --overlap --batch_size 12 --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 4 --lr 0.001 --epochs 60 --method ${METHOD} --opt_level O1 ${OPTIONS} --model 'segformer_b2' --distill_weight_args 30 --ckpt checkpoints_model/step/50-10-ade_Innovation-v4_4_segformer_b2.pth
